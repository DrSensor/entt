...
TEST(Poly, Functionalities) {
    auto test = [](auto identity) {
        using poly_type = typename decltype(identity)::type;

        impl instance{};

        poly_type empty{};
        poly_type in_place{std::in_place_type<impl>, 3};
        poly_type alias{std::ref(instance)};
    ...
    test(entt::type_identity<entt::poly<Deduced>>{});
    test(entt::type_identity<entt::poly<Defined>>{});
}
...
TEST(Poly, Alias) {
    auto test = [](auto identity) {
        ...
        impl instance{};
        poly_type poly{std::ref(instance)};
